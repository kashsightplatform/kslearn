...
Usage:
    ...
    python ksl_tool.py pack file.json -t "Title"
"""
...
TYPE_HIERARCHICAL = "hierarchical"  # Course → Category → Unit → Outcome → Sub-topic → Content
...
def _build_header(content_type: str, title: str, author: str,
    ...
    header[8:24] = _pack_string(content_type, 16)
    header[24:56] = _pack_string(title, 32)
    header[56:72] = _pack_string(author, 16)
    struct.pack_into(">Q", header, 72, int(datetime.now().timestamp()))
    struct.pack_into(">Q", header, 80, payload_len)
    header[88:124] = AUTHOR_CRED
    struct.pack_into(">I", header, 124, content_count)
    return bytes(header)
...
def detect_content_type(data: dict) -> str:
    ...
    if "courses" in data:
        types_found.add(TYPE_HIERARCHICAL)
...
def pack_ksl(input_files: list, output_path: Path = None, title: str = None,
    ...
    for fpath in input_files:
        ...
        for key in ["notes", "quiz_topics", "quiz_metadata", "flashcards", "tutorials", "brain_qa", "courses"]:
            if key in data:
...
def unpack_ksl(ksl_path: Path, output_dir: Path = None) -> dict:
    ...
    if output_dir:
        ...
        name_map = {TYPE_BRAIN: "_brain", TYPE_NOTES: "_notes", TYPE_QUIZ: "_quiz", TYPE_HIERARCHICAL: "_hierarchical"}
...
def _interactive_pack():
    ...
    print("  [3] 📚 Hierarchical — Course → Category → Unit → Outcome → Content")
    print()

    pkg_type = input("╰─► Choose type [1, 2, or 3]: ").strip()
    if pkg_type == "2":
        build_type = TYPE_BRAIN
    elif pkg_type == "3":
        build_type = TYPE_HIERARCHICAL
    else:
        build_type = TYPE_COMBINED
    ...
    elif build_type == TYPE_HIERARCHICAL:
        print("Expected: files with 'courses' key (hierarchical structure).")
        print("See HIERARCHICAL_FORMAT.md for the full schema.")
    else:
    ...
    if build_type == TYPE_BRAIN:
        ...
        _build_hierarchical_ksl(title, all_data, json_files, out_path)
    else:
...
def _build_hierarchical_ksl(title: str, all_data: dict, json_files: list, out_path: Path):
    """Build a hierarchical .ksl package with 6-level navigation."""
    courses_data = all_data["content"].get("courses", [])
    if not courses_data:
        print("❌ No 'courses' data found!")
        print("   JSON must have a 'courses' key with hierarchical structure.")
        sys.exit(1)
    total_units = 0
    total_outcomes = 0
    total_content = 0
        ...
        for cat in categories:
            ...
            for unit in units:
                ...
                for outcome in outcomes:
                    ...
                    total_content += len(subtopics)
    if total_units == 0:
        print("❌ No units found in courses!")
        print("   Structure: courses → categories → units → learning_outcomes → subtopics → content")
        sys.exit(1)
    hier_pkg = {
        "ksl_version": 1,
        "metadata": dict(all_data["metadata"]),
    ...
    hier_pkg["metadata"]["content_type"] = TYPE_HIERARCHICAL
    hier_pkg["metadata"]["title"] = title
    hier_json = json.dumps(hier_pkg, indent=2, ensure_ascii=False).encode("utf-8")
    header = _build_header(TYPE_HIERARCHICAL, title, "kslearn", len(hier_json), total_content)
    with open(out_path, "wb") as f:
        f.write(header)
        f.write(hier_json)
    print()
    print("=" * 60)
    print("✅ Hierarchical KSL Package Created")
    print("=" * 60)
    print(f"\n  📚 Hierarchical (Course → Category → Unit → Outcome → Sub-topic → Content)")
    print(f"     File:    {out_path}")
    print(f"     Size:    {out_path.stat().st_size:,} bytes")
    print(f"     Courses: {len(courses_data)}")
    print(f"     Units:   {total_units}")
    print(f"     Outcomes:{total_outcomes}")
    print(f"     Content: {total_content} sub-topics")
    print(f"     Source:  {json_files[0].name}")
...
def main():
    ...
    p = sub.add_parser("pack", help="Pack JSON file(s) into .ksl")
    p.add_argument("files", nargs="+", help="JSON file(s) to pack")
    p.add_argument("-o", "--output", help="Output .ksl path")
    ...
    if args.command == "pack":
        ...
        try:
            ...
            if isinstance(data, dict):
                for key in ["notes", "quiz_topics", "flashcards", "tutorials", "brain_qa", "courses"]:
                    if key in data:
